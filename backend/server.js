...
app.post("/api/public/orders", async (req, res) => {
  try {
    ...
    const itemSummary = items.map((item) => {
      ...
      return item.name + " | ID: " + (item.id || 0) + " | Size: " + (item.size || "M") + (item.color ? " | Colour: " + item.color : "") + " | Qty: " + qty + " | Rs. " + price.toFixed(2) + " | Total: Rs. " + (price * qty).toFixed(2);
    }).join("\n");
...
(async function () {
  ...
  var fashionKB = {
    ...
    coupon_response: "Check your email for exclusive promo codes after signup! We regularly send special offers to our customers."
  };
